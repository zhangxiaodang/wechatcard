	...
	public static Yufuka openaccount(Yufuka yufuka, BranchparametersForm _branchInfoform) {
		...
		try {
			...
			String timestamp = _sf.format(new Date());// 时间戳
 		    String signstring = "" + app_secret 
 				    + "app_key" + app_key 
					+ "brand_no"+ yufuka.getBrand_no()  
					+ "brh_id" + yufuka.getBrh_id() 
					+ "chan_no" + yufuka.getChan_no() 
					+ "formatxml"
					+ "methodallinpay.ppcs.cloud.card.open"
					+ "order_id" + yufuka.getOrder_id() 
					+ "password" + yufuka.getPassword() 
					+ "phone_num" + yufuka.getPhone_num()
					+ "sign_methodmd5" 
					+ "sign_v1" 
					+ "timestamp" + timestamp 
					+ "v" + api_v + "" 
		 ...
		 System.out.println("signstring="+signstring);
 		 ...
 		 String md5sign = MD5.signature(signstring);// 对字符串进行md5加密
			String url = befurl 
					...
					+ "&chan_no=" + yufuka.getChan_no()
					+ "&format=xml"
					+ "&method=allinpay.ppcs.cloud.card.open"
					+ "&order_id="+ yufuka.getOrder_id() 
					+ "&password=" + yufuka.getPassword()
					+ "&phone_num=" + yufuka.getPhone_num() 
					+ "&sign=" + md5sign
	...
	public static CalculateinterestForm getCardinfoAPI(String strcardid, String strpassword,
		...
		try {
			...
			String signstring = "" + app_secret + "app_key" + app_key + "card_id" + strcardid
					+ "formatxmlmethodallinpay.card.cardinfo.getpassword" + _DesInfo + "sign_methodmd5sign_v1timestamp"
